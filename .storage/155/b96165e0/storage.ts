...
// Default VIP user account for testing
const defaultVipUser: User = {
  id: '3',
  username: 'vipuser',
  password: 'vip123', // In real app, this would be hashed
  role: 'user',
  type: 'vip',
  name: 'VIP User',
  email: 'vip@rajarepair.com',
  phone: '089812345678',
  address: 'Jl. VIP No. 3, Cirebon',
  csCode: 'VIP001',
  createdAt: new Date().toISOString()
};
...
export function initializeStorage() {
  ...
  if (!localStorage.getItem(STORAGE_KEYS.USERS)) {
    localStorage.setItem(STORAGE_KEYS.USERS, JSON.stringify([defaultAdmin, defaultUser, defaultVipUser]));
  }
...
export function addTransaction(transaction: Transaction): void {
  ...
  // Only update product stock if transaction status indicates completed fulfillment
  if (transaction.status === 'completed' || transaction.status === 'shipping' || transaction.status === 'ready_for_pickup') {
    updateStockFromTransaction(transaction);
  }
}

function updateStockFromTransaction(transaction: Transaction): void {
  if (transaction.status !== 'completed' && transaction.status !== 'shipping' && transaction.status !== 'ready_for_pickup') return;
...
export function updateTransaction(updatedTransaction: Transaction): boolean {
  ...
  if (index !== -1) {
    ...
    // Check if the status changed in a way that affects inventory
    const oldStatusAffectsInventory = oldTransaction.status === 'completed' || 
                                     oldTransaction.status === 'shipping' || 
                                     oldTransaction.status === 'ready_for_pickup';
                                     
    const newStatusAffectsInventory = updatedTransaction.status === 'completed' || 
                                     updatedTransaction.status === 'shipping' || 
                                     updatedTransaction.status === 'ready_for_pickup';
    
    // If status changed from non-inventory affecting to inventory affecting, update stock
    if (!oldStatusAffectsInventory && newStatusAffectsInventory) {
      updateStockFromTransaction(updatedTransaction);
    }
    // If status changed from inventory affecting to non-inventory affecting, revert stock changes
    else if (oldStatusAffectsInventory && !newStatusAffectsInventory) {
...
export function updateTransactionStatus(transactionId: string, status: TransactionStatus): boolean {
  ...
  if (index !== -1) {
    ...
    // For outgoing transactions, validate stock availability when moving to statuses that affect inventory
    if (updatedTransaction.type === 'out' && 
       (status === 'completed' || status === 'shipping' || status === 'ready_for_pickup')) {
      if (!canProcessTransaction(updatedTransaction)) {
...
export function exportProductsToCSV(): string {
  ...
  // CSV header
  let csvContent = "Code,Name,Category,Price,VIP Price,Stock\n";
  ...
  products.forEach(product => {
    ...
    const row = [
      ...
      `"${categoryName}"`,
      product.price,
      product.vipPrice || Math.round(product.price * 0.9), // Default VIP price if not set
      product.stock
    ].join(",");
    csvContent += row + "\n";
  });
...
export function parseProductsFromCSV(csvData: string, categoryNameToIdMap: {[key: string]: string}): Product[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (fields.length >= 5) {
      ...
      const vipPriceStr = fields[5] || ''; // Optional VIP price column
      
      // Get category ID from name
      const categoryId = categoryNameToIdMap[categoryName.replace(/"/g, '')] || defaultCategories[0].id;
      
      const regularPrice = parseFloat(priceStr) || 0;
      // If VIP price is provided, use it; otherwise set it at 90% of regular price
      const vipPrice = vipPriceStr ? parseFloat(vipPriceStr) : Math.round(regularPrice * 0.9);
...
// Get product price based on user type
export function getPriceForUser(product: Product, userType: UserType = 'regular'): number {
  if (userType === 'vip' && product.vipPrice !== undefined) {
    return product.vipPrice;
  }
  return product.price;
}

// Add product to cart (kept in localStorage)
...
export function getCart(): { productId: string; quantity: number }[] {
  const cart = localStorage.getItem('raja-repair-cart');
  return cart ? JSON.parse(cart) : [];
}

// Clear the cart
export function clearCart(): void {
  localStorage.removeItem('raja-repair-cart');
}

// Update user type (VIP status)
export function updateUserType(userId: string, newType: UserType): boolean {
  const users = getUsers();
  ...
  if (index !== -1) {
    ...
    // If this is the current user, update in local storage
