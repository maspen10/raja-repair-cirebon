...
import { addTransaction, getProducts, generateId, getCurrentUser, getDeliveryMethods, getPaymentMethods, canProcessTransaction, getTransactions } from '@/lib/storage';
...
export function TransactionForm({ type, onSuccess, onCancel }: TransactionFormProps) {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      // Check for duplicate orders (same product in pending or payment_confirmed status)
      if (type === 'out') {
        const allTransactions = getTransactions();
        const userPendingOrders = allTransactions.filter(t => 
          t.userId === currentUser.id && 
          t.type === 'out' && 
          (t.status === 'pending' || t.status === 'payment_confirmed' || t.status === 'processing')
        );
        
        // Check if any product in the current order is also in a pending/confirmed order
        const duplicateItems: string[] = [];
        
        for (const item of values.items) {
          for (const order of userPendingOrders) {
            if (order.items.some(orderItem => orderItem.productId === item.productId)) {
              const product = products.find(p => p.id === item.productId);
              if (product) {
        ...
        if (duplicateItems.length > 0) {
          toast({
            ...
            description: `Anda sudah memiliki pesanan yang belum selesai untuk: ${duplicateItems.join(', ')}. Silakan selesaikan pesanan yang ada terlebih dahulu.`,
          });
          setIsLoading(false);
          return;
        }
      ...
      if (type === 'out' && !canProcessTransaction(newTransaction)) {
        toast({
          ...
          description: 'Satu atau lebih produk memiliki stok yang tidak mencukupi.',
        });
        setIsLoading(false);
        return;
      }
      
      // Make sure we actually process the transaction
      addTransaction(newTransaction);
      
      toast({
        title: type === 'in' ? 'Transaksi Berhasil' : 'Pesanan Berhasil Dibuat',
        description: type === 'in' 
          ? 'Produk telah ditambahkan ke inventaris.' 
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>
          {type === 'in' ? 'Pencatatan Produk Masuk' : 'Buat Pesanan'}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
            ...
            {step === 'delivery' && type === 'out' && (
              <div className="space-y-4">
                        ...
                        <SelectContent>
                          {deliveryMethods.map((method) => (
                            <SelectItem key={method.id} value={method.id}>
                              {method.name} {method.cost > 0 ? `(${formatPrice(method.cost)})` : '(Gratis)'}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                ...
                {form.watch('deliveryMethodId') && 
                  !deliveryMethods.find(m => m.id === form.watch('deliveryMethodId'))?.name.toLowerCase().includes('pick-up') && (
                  <FormField
                    control={form.control}
                  ...
                  <div className="border rounded-md p-4 bg-muted/50">
                    {(() => {
                      ...
                      if (selectedMethod.name.toLowerCase().includes('cash')) {
                        return (
                          <p className="text-sm">
                            Bayar dengan tunai saat mengambil pesanan Anda.
                          </p>
                        );
                      }
                      ...
                      return (
                        <div className="space-y-2 text-sm">
                          ...
                          {selectedMethod.accountName && (
                            <p>Nama akun: {selectedMethod.accountName}</p>
                          )}
                          {selectedMethod.accountNumber && (
                            <p>Nomor akun: {selectedMethod.accountNumber}</p>
                          )}
                          {selectedMethod.description && (
                            <p>{selectedMethod.description}</p>
                          )}
                          <div className="pt-2">
                            <p className="font-medium">Jumlah yang harus dibayar: {formatPrice(totalAmount + deliveryCost)}</p>
                          </div>
                          
                          {!selectedMethod.name.toLowerCase().includes('cash') && (
                            <div className="pt-3">
                              <p className="font-medium">Unggah Bukti Pembayaran:</p>
                              <div className="flex flex-col gap-2 mt-2">
                                <div className="flex items-center gap-2">
                                  ...
                                  />
                                  {paymentProofUrl && (
                                    <span className="text-sm text-green-600">Bukti pembayaran terunggah</span>
                                  )}
                                </div>
            ...
            {step === 'confirmation' && (
              <div className="space-y-4">
                <div className="border rounded-md p-4">
                  <h3 className="text-lg font-semibold mb-3">{type === 'in' ? 'Ringkasan Transaksi' : 'Ringkasan Pesanan'}</h3>
                  
                  <div className="space-y-3">
                    <p className="font-medium">Produk yang Dipilih:</p>
                ...
                {type === 'out' && (
                  <div className="border rounded-md p-4">
                    <h3 className="text-lg font-semibold mb-3">Informasi Pengiriman & Pembayaran</h3>
                    
                    <div className="space-y-3">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <p className="text-muted-foreground text-sm">Metode Pengiriman:</p>
                ...
                {type === 'out' && (
                  <div className="p-4 border rounded-md bg-muted/50">
                    <p className="text-sm">
                      <span className="font-semibold">Catatan:</span> Setelah membuat pesanan, admin akan meninjau dan memprosesnya. Anda dapat memeriksa status pesanan di riwayat pesanan.
                    </p>
                  </div>
                )}
              </div>
        ...
        {step === 'items' ? (
          ...
          <>
            ...
            <Button 
              ...
              !paymentMethods.find(m => m.id === form.getValues('paymentMethodId'))?.name.toLowerCase().includes('cash'))}
            >
              Tinjau Pesanan
            </Button>
          </>
          ...
          <>
            ...
            <Button onClick={form.handleSubmit(onSubmit)} disabled={isLoading}>
              {isLoading ? 'Memproses...' : type === 'in' ? 'Catat Transaksi' : 'Buat Pesanan'}
            </Button>
          </>
        )}
      </CardFooter>
